...
# Processing Directories with `histBatch`

## Generating a file list in R

If you have a large number of files to process, `flowHist` can read them
all in together. The `histBatch` function provides this feature. To use it,
you need a list of file names to process. The R function `list.files()` is
helpful here. If your FCS files are in the directory "`datadir/`", you can
collect all their names using:
...
The `histBatch()` function has two required arguments:
...
Continue on to file 8, `337.LMD`. Here, the model fit is actually not too bad, but the G1 peak of sample A has been erroneously assigned to noise in the debris field:
